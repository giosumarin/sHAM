...
@click.option('--tr', default=0)#0.001, help='treshold for ECSQ')
...
def main(compression, net, dataset, learning_rate, lr_cumulative, minibatch, prfc, prcnn, clusterfc, clustercnn, tr, lambd, logger, ptnc):
    ...
    if compression == 'pr':
        ...
        lambdas = [1e-13, 2.5e-13, 5e-13, 7.5e-13,1e-12, 2.5e-12, 5e-12, 7.5e-12, 1e-11, 2.5e-11, 5e-11, 7.5e-11, 1e-10, 2.5e-9, 5e-9, 7.5e-9, 1e-9, 5e-8][::-1]
    ...
    if compression == "pr":
        ...
        compression_model.train_ws(epochs=20, lr=lr_cumulative, dataset=dataset, X_train=x_train, y_train=y_train, X_test=x_test, y_test=y_test, patience=ptnc, min_is_better=False, threshold=tr, step_per_epoch=step_per_epoch, pruPWS_train=True if compression == "pruPWS" else False)
